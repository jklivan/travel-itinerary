...
function evaluateSourceNode(path, predicate, expression, values) {
  ...
  const result = vm.runInNewContext(code, { ...values, getRecommendation, recommendationTags })
  return JSON.parse(JSON.stringify(result))
}

test('guided creation saves hotel, restaurant and activity recommendation tags', () => {
  const items = [
    { id: 'hotel', type: 'hotel', name: 'Hotel', dayIndex: 1, tags: ['Boutique'], isHighlight: true },
    { id: 'food', type: 'food_drink', name: 'Cafe', dayIndex: 1, tags: ['Local Favorite', '__avoid'], isHighlight: false },
    { id: 'activity', type: 'activity', name: 'Tour', dayIndex: 1, tags: ['__highlight'], isHighlight: true },
  ]
