...
try {
 ...
 await composer.getByRole('button',{name:'Post for 24 hours',exact:true}).click()
 await page.getByRole('status').waitFor()
 const standalone=await page.evaluate(()=>window.posted)
 assert.equal(standalone.itemId,undefined);assert.equal(standalone.placeName,'Lakeside walk');assert.equal(standalone.destination,'Lucerne, Switzerland');assert.equal(standalone.type,'activity');assert.equal(standalone.newPlanTitle,'Lucerne weekend');assert.match(standalone.newPlanId,/^[a-f0-9-]{36}$/);assert.equal(standalone.photoUrl,'/uploaded.jpg')
 console.log('PASS: existing-trip moments, new itinerary moments, uploads, retry, and success confirmation.')
