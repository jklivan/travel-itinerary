...
test('photo lookup keeps author attribution, uses no-store and never returns the API key', async () => {
  ...
  try {
    ...
    const maps = new URL(photo.mapsUrl)
    assert.equal(maps.origin, 'https://www.google.com')
    assert.equal(maps.pathname, '/maps/search/')
    assert.equal(maps.searchParams.get('query_place_id'), 'nobu')
    assert.equal(maps.searchParams.get('query'), 'Nobu Ibiza Bay')
    assert.notEqual(photo.mapsUrl, photo.authors[0].uri)
    assert.notEqual(photo.mapsUrl, photo.url)
