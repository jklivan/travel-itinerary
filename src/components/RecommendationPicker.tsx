    ...
    <fieldset className="space-y-1.5">
      ...
      <div className="flex flex-wrap gap-2">
        <button type="button" aria-pressed={value === 'must'} onClick={() => onChange(value === 'must' ? 'none' : 'must')}
          className={`inline-flex min-h-10 items-center gap-1.5 rounded-lg border px-3 text-sm font-medium ${value === 'must' ? 'border-[#59694f] bg-[#59694f] text-white' : 'border-gray-300 text-gray-600 hover:border-[#59694f]'}`}><Icon size={15} />{label}</button>
        ...
        <button type="button" aria-pressed={value === 'option'} onClick={() => onChange(value === 'option' ? 'none' : 'option')}
          className={`inline-flex min-h-10 items-center gap-1.5 rounded-lg border px-3 text-sm font-medium ${value === 'option' ? 'border-[#59694f] bg-[#59694f] text-white' : 'border-gray-300 text-gray-600 hover:border-[#59694f]'}`}><Bookmark size={15} />Save as alternative</button>
      </div>
