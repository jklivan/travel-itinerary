...
export default function MessageComposer({ recipientId, itineraryId, attachment, replyTo, onClearReply }: { recipientId: string; itineraryId?: string; attachment?: ComposerAttachment; replyTo?: MessageReplyTarget; onClearReply?: () => void }) {
  ...
  return <form className="mt-6 space-y-3 rounded-xl border border-[#d7cebc] bg-[#faf7f1] p-4 shadow-sm" onSubmit={event => {
    ...
    {replyTo && <div className="rounded-lg border-l-4 border-[#59694f] bg-[#e6ece5] p-3 text-sm">
      <p className="font-semibold text-[#59694f]">Replying to {replyTo.author}</p>
      {(replyTo.placeName || replyTo.itineraryTitle) && <p className="mt-1 text-xs text-[#59694f]">{[replyTo.placeName, replyTo.itineraryTitle].filter(Boolean).join(' · ')}</p>}
    ...
    <label className="block text-sm font-medium text-[#2e4147]">Private message
      <textarea ref={textarea} required={!selected} maxLength={4000} rows={4} value={content} disabled={pending} onChange={event => { setContent(event.target.value); clientId.current = null }} placeholder={replyTo ? 'Write your reply…' : 'Write a message…'} className="mt-2 w-full rounded-xl border-2 border-[#8caaa3] bg-[#fffdf6] p-3 text-base leading-relaxed placeholder:text-[#7a7b70] focus:outline-none focus:ring-2 focus:ring-[#59694f]/25 focus:border-[#59694f]" />
    </label>
    <button type="button" aria-label="Attach an itinerary" aria-expanded={showPicker} disabled={pending} onClick={() => setShowPicker(value => !value)} className="inline-flex min-h-11 items-center gap-2 rounded-full border border-[#8caaa3] px-3 text-sm font-medium text-[#59694f] hover:bg-[#e6ece5] disabled:opacity-50"><span aria-hidden="true" className="text-2xl leading-none">+</span> Add itinerary</button>
    ...
    <button disabled={pending || (!content.trim() && !selected)} className="rounded-full bg-[#59694f] px-5 py-2.5 text-sm font-medium text-white hover:bg-[#355650] transition-colors disabled:opacity-50">{pending ? 'Sending…' : 'Send message'}</button>
  </form>
}
