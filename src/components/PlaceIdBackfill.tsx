...
export default function PlaceIdBackfill() {
  ...
  return <section className="mb-8 rounded-xl border border-gray-200 bg-white p-5">
    ...
    <div className="mt-4 flex flex-wrap gap-3"><button type="button" disabled={running} onClick={() => void run(false)} className="min-h-11 rounded-lg border px-4 text-sm disabled:opacity-50">Preview matches</button><button type="button" disabled={running} onClick={() => void run(true)} className="min-h-11 rounded-lg bg-[#59694f] px-4 text-sm text-white disabled:opacity-50">Fill missing IDs</button>{running && <button type="button" onClick={() => { stopped.current = true }} className="min-h-11 px-3 text-sm underline">Stop after this batch</button>}</div>
