    ...
    <div className="rounded-sm border border-[#d7dcd0] border-t-2 border-t-[#507c76] bg-[#fffdf6] p-3 shadow-sm">
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0">
          <p className="text-[10px] font-semibold uppercase tracking-widest text-[#507c76]">{kind === 'trip' ? 'About this trip' : 'Place from the trip'}</p>
      ...
      {href && <Link className="mt-3 inline-block text-sm font-semibold text-[#507c76] hover:underline" href={href}>{kind === 'trip' ? 'View trip →' : 'View place in trip →'}</Link>}
    </div>
  )
}
