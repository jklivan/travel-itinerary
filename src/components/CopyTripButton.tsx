...
export default function CopyTripButton({ itineraryId, title, isOwn }: { itineraryId: string; title: string; isOwn: boolean }) {
  ...
  return <>
    <button type="button" onClick={() => { setError(''); dialog.current?.showModal() }} className="min-h-11 rounded-full border border-[#8caaa3] bg-[#fffdf7] px-4 py-2 text-sm font-semibold text-[#59694f]">Copy trip</button>
    <dialog ref={dialog} aria-labelledby={headingId} onCancel={event => { if (saving.current) event.preventDefault() }} className="fixed inset-0 m-auto max-h-[85dvh] w-[calc(100%-2rem)] max-w-md overflow-y-auto rounded-2xl border border-[#d7cebc] bg-[#faf7f1] p-5 text-[#2e4147] shadow-xl backdrop:bg-black/50">
      ...
      <form className="mt-5 space-y-4" onSubmit={async event => {
        ...
        <fieldset disabled={busy} className="space-y-4">
          ...
          <label className="flex min-h-11 items-center gap-3 text-sm"><input type="checkbox" checked={keepDays} onChange={event => setKeepDays(event.target.checked)} className="h-5 w-5 accent-[#59694f]" />Keep the day-by-day layout</label>
          {isOwn && <label className="flex min-h-11 items-center gap-3 text-sm"><input type="checkbox" checked={keepNotes} onChange={event => setKeepNotes(event.target.checked)} className="h-5 w-5 accent-[#59694f]" />Include my notes</label>}
