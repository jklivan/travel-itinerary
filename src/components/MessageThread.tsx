...
export default function MessageThread({ messages, userId, person, itineraryId, attachment }: { messages: Messages; userId: string; person: { id: string; name: string }; itineraryId?: string; attachment?: ComposerAttachment }) {
  ...
  return <>
    <div className="space-y-4">{messages.map(message => <article id={`message-${message.id}`} key={message.id} className={`rounded-xl border border-[#d7cebc] p-4 shadow-sm ${message.senderId === userId ? 'ml-6 rounded-br-sm bg-[#e6ece5]' : 'mr-6 rounded-bl-sm bg-[#faf7f1]'}`}>
      <p className="text-xs font-semibold text-[#59694f] mb-2">{message.senderId === userId ? 'You' : person.name}</p>
      {message.replyTo && <blockquote className="mb-3 rounded-md border-l-4 border-[#59694f] bg-white/50 p-3 text-sm text-[#6b7067]">
        <p className="font-semibold text-[#59694f]">Reply to {message.replyTo.senderId === userId ? 'you' : person.name}</p>
      ...
      <div className="mt-3 flex items-center justify-between gap-3">
        ...
        <button type="button" className="shrink-0 rounded-full border border-[#8caaa3] px-3 py-1.5 text-sm font-medium text-[#59694f] hover:bg-[#e6ece5]" onClick={() => setReplyTo({ id: message.id, content: message.content, author: message.senderId === userId ? 'yourself' : person.name, itineraryTitle: message.itineraryTitle, placeName: message.placeName })}>Reply</button>
      </div>
    </article>)}</div>
