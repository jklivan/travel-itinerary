...
export default function StoryComposer({ onClose, onPosted, initialItemId }: { onClose: () => void; onPosted: () => void; initialItemId?: string }) {
  ...
  function selectPlace(id: string) {
    ...
    if (id === '__new__') { setNewName(''); setNewPlaceId(''); setNewDestination(trip?.places[0]?.destination ?? ''); setNewCountry('') }
  }
  ...
  return <dialog ref={dialog} className={styles.composer} aria-labelledby={titleId} onClose={onClose} onCancel={event => { if (busy || uploading) event.preventDefault() }}>
    ...
    <form onSubmit={async event => {
      ...
      if (mode === 'new' || addingNewPlace) {
        const missing = [!newPlanTitle.trim() && mode === 'new' && activityPlan === 'new' ? 'an itinerary title' : '', !newDestination.trim() ? 'a destination' : '', !newName.trim() ? 'an activity name' : ''].filter(Boolean)
      ...
      try {
        ...
        const shared = mode === 'trip' && place
          ...
          : { placeName: newName.trim(), destination: newDestination.trim(), country: newCountry, type: newType, placeId: newPlaceId, ...(mode === 'new' && activityPlan === 'new' ? { newPlanId: newPlanId.current, newPlanTitle } : { tripId: mode === 'trip' ? tripId : activityPlan }), caption }
