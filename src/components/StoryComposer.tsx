...
export default function StoryComposer({ onClose, onPosted, initialItemId }: { onClose: () => void; onPosted: () => void; initialItemId?: string }) {
  ...
  function selectPlace(id: string) {
    ...
    setItemId(id); setUploaded([]); setSelectedPhotos([selected?.photos[0] ?? trip?.photos[0] ?? ''].filter(Boolean)); setError(''); clientIds.current = null
  }
    ...
    <form onSubmit={async event => {
      ...
      try {
        ...
        const shared = mode === 'trip'
          ...
          : { placeName: newName.trim(), destination: newDestination.trim(), country: newCountry, type: newType, placeId: newPlaceId, ...(activityPlan === 'new' ? { newPlanId: newPlanId.current, newPlanTitle } : { tripId: activityPlan }), caption }
      ...
      {sources && <fieldset disabled={busy || uploading} className={styles.fields}>
        <div className="flex gap-2" role="tablist" aria-label="Moment source">
          <button type="button" role="tab" aria-selected={mode === 'new'} onClick={() => { setMode('new'); setSelectedPhotos([]); setUploaded([]); setError(''); clientIds.current = null }} className={`min-h-11 flex-1 rounded-full border px-3 text-sm ${mode === 'new' ? 'border-[#59694f] bg-[#e6ece5] text-[#2e4147]' : 'border-[#d7cebc]'}`}>New activity</button>
          {!!sources.trips.length && <button type="button" role="tab" aria-selected={mode === 'trip'} onClick={() => { setMode('trip'); setSelectedPhotos([]); setUploaded([]); setError(''); clientIds.current = null }} className={`min-h-11 flex-1 rounded-full border px-3 text-sm ${mode === 'trip' ? 'border-[#59694f] bg-[#e6ece5] text-[#2e4147]' : 'border-[#d7cebc]'}`}>From a trip</button>}
        </div>
        {mode === 'trip' ? <>
          <label>Your trip<select value={tripId} onChange={event => { setTripId(event.target.value); setItemId(''); setSelectedPhotos([]); setUploaded([]); clientIds.current = null }}>{sources.trips.map(trip => <option key={trip.id} value={trip.id}>{trip.title}</option>)}</select></label>
        ...
        {(mode === 'trip' ? !!place : true) && <>
          ...
          {selectedPhotos[0] && <div className={styles.preview}><div className={styles.paper}>
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src={selectedPhotos[0]} alt={`Story preview for ${mode === 'trip' ? place?.name : newName}`} /><h3>{mode === 'trip' ? place?.name : newName}</h3><p>{mode === 'trip' ? place?.destination : [newDestination, newCountry].filter(Boolean).join(', ')}</p>{caption && <p className={styles.caption}>{caption}</p>}{selectedPhotos.length > 1 && <p className={styles.caption}>+ {selectedPhotos.length - 1} more story photos</p>}
          </div></div>}
          <p className={styles.privacy}>{mode === 'trip' ? `${selectedPhotos.length} photo${selectedPhotos.length === 1 ? '' : 's'} will also be saved to ${place?.name} in your trip.` : `This activity and ${selectedPhotos.length} photo${selectedPhotos.length === 1 ? '' : 's'} will be added to ${activityPlan === 'new' ? newPlanTitle || 'your new private itinerary' : 'your itinerary'}.`}</p>
          ...
          <button type="submit" className={styles.post} disabled={!selectedPhotos.length || selectedPhotos.length > 10 || (mode === 'trip' && !place) || busy || uploading}>{busy ? 'Posting…' : `Post ${selectedPhotos.length > 1 ? `${selectedPhotos.length} photos` : 'for 24 hours'}`}</button>
        </>}
      </fieldset>}
