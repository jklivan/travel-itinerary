      ...
      <dialog ref={dialog} onCancel={event => { if (pending) event.preventDefault() }} aria-labelledby={titleId} className="m-auto w-[calc(100%_-_2rem)] max-w-sm rounded-2xl bg-[#faf7f1] p-5 text-[#242e25] shadow-xl backdrop:bg-black/40">
        ...
        <form onSubmit={event => {
          ...
          <label className="block text-sm">Folder
            <select value={folderId} onChange={event => setFolderId(event.target.value)} disabled={!loaded || pending} className="mt-2 w-full rounded-lg border border-[#c1ad93] bg-white p-3">
          ...
          {folderId === 'new' && <label className="mt-4 block text-sm">New folder name
            <input autoFocus required maxLength={80} value={name} onChange={event => setName(event.target.value)} placeholder="Beach ideas" disabled={pending} className="mt-2 w-full rounded-lg border border-[#c1ad93] bg-white p-3" />
          </label>}
