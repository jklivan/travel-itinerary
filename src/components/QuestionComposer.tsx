...
export default function QuestionComposer({ questionId }: { questionId?: string }) {
  ...
  return <form className="space-y-3 rounded-2xl border border-[#C4A882] bg-[#FAF7F2] p-5" onSubmit={event => {
    ...
    </div> : <button type="button" disabled={pending} onClick={() => setShowPicker(!showPicker)} className="text-sm font-medium text-[#507c76] underline">{showPicker ? 'Cancel itinerary search' : '+ Tag an itinerary'}</button>}
    {showPicker && !trip && <div className="space-y-2 rounded-xl border border-[#d7cebc] bg-[#fffdf6] p-3">
      ...
      <button type="button" disabled={pending || searching || query.trim().length < 2} className="rounded-full border border-[#8caaa3] px-3 py-1.5 text-sm text-[#507c76] disabled:opacity-50" onClick={() => {
        setError('')
        startSearch(async () => {
    ...
    <button disabled={pending || searching || !content.trim()} className="rounded-full bg-[#507c76] px-5 py-2.5 text-sm font-medium text-white hover:bg-[#355650] disabled:opacity-50">{pending ? 'Posting…' : questionId ? 'Post reply' : 'Ask your friends'}</button>
  </form>
}
