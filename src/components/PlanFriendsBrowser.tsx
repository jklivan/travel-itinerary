...
export default function PlanFriendsBrowser({ plan, initialQuery, initialResults }: {
  ...
  return <div className="mx-auto max-w-2xl px-4 py-6 pb-64 text-[#2e4147]">
    ...
    <form className="mt-5" onSubmit={event => { event.preventDefault(); void search(query) }}>
      ...
      <div className="mt-2 flex gap-2"><input id="friends-destination" value={query} maxLength={160} required disabled={loading || saving} onChange={event => setQuery(event.target.value)} placeholder="London" className="min-w-0 flex-1 rounded-xl border border-[#8caaa3] bg-[#fffdf7] p-3 text-base" /><button disabled={loading || saving || !query.trim()} className="min-h-11 rounded-xl bg-[#507c76] px-4 text-sm font-semibold text-white disabled:opacity-50">{loading ? 'Searching…' : 'Find trips'}</button></div>
    </form>
