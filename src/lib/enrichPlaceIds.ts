...
export async function enrichPlaceIds(ids: string[]) {
  const items = await prisma.destItem.findMany({ where: { type: { not: 'transport' }, id: { in: ids }, OR: [{ placeId: null }, { placeId: '' }] }, select: { id: true, name: true, lat: true, lng: true, destinationId: true, destination: { select: { name: true, country: true, lat: true, lng: true } } } })
