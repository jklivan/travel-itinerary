...
export default async function ConversationPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ place?: string; before?: string }> }) {
  ...
  if (!session?.user?.id) redirect(`/login?callbackUrl=${encodeURIComponent(`/messages/${id}${placeId ? `?place=${encodeURIComponent(placeId)}` : ''}`)}`)
  ...
  return <div className="max-w-2xl mx-auto px-4 py-6">
    ...
    {result.hasOlder && <Link className="block mb-4 text-sm underline" href={`/messages/${id}?before=${result.messages[0].id}`}>Earlier messages</Link>}
    {before && <Link href={`/messages/${id}`} className="block mb-4 text-sm underline">Latest messages</Link>}
