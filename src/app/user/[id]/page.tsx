          ...
          <section aria-labelledby="your-trips-heading">
            ...
            <section className="mt-5" aria-labelledby="private-plans-heading">
              ...
              {drafts.length === 0 ? <div className="rounded-2xl border border-dashed border-[#d7cebc] p-6 text-center text-sm text-[#73786d]">No private plans yet.</div> : <div className="space-y-4">{drafts.map(trip => <div key={trip.id} className="overflow-hidden rounded-2xl border border-[#e1d8c9] bg-[#fffdf7] p-4 shadow-[0_2px_8px_rgba(45,38,27,0.08)]">
                <Link href={trip.isPlan ? `/plan/${trip.id}` : `/itinerary/${trip.id}/edit`} className="flex min-w-0 items-center gap-4">
                  <span className="relative size-[6.4rem] shrink-0 rotate-[-4deg] overflow-hidden border-[5px] border-white bg-[#e8eee8] shadow-[0_2px_4px_rgba(45,38,27,0.18)]">{draftCoverPhotos.get(trip.id) ? <Image src={draftCoverPhotos.get(trip.id)!} alt="" fill sizes="102px" className="object-cover" /> : <span className="grid h-full place-items-center text-center text-[9px] uppercase tracking-wider text-[#59694f]">Postcard</span>}</span>
                  <span className="min-w-0"><span className="block break-words font-[family-name:var(--font-playfair)] text-lg leading-tight text-[#2e4147]">{trip.title || 'Untitled trip'}</span><span className="mt-2 block text-[11px] font-semibold uppercase tracking-[0.12em] text-[#73786d]">{trip.destinations.reduce((sum, destination) => sum + destination.items.length, 0)} places</span><span className="mt-1 block text-sm text-[#73786d]">Keep planning →</span></span>
                </Link>
                <div className="mt-3 flex items-center justify-between gap-3"><DeleteButton id={trip.id} visibility={trip.visibility} returnTo={`/user/${id}`} label="Delete trip" /><Link href={`/plan/${trip.id}`} aria-label="Post trip" title="Post trip" className="relative inline-flex h-14 w-20 items-center justify-center rounded-md bg-[#355650] shadow-sm [clip-path:polygon(7%_0,93%_0,100%_10%,100%_90%,93%_100%,7%_100%,0_90%,0_10%)]"><span className="flex h-11 w-16 items-center justify-center border-2 border-dashed border-[#355650] bg-[#f1e7d8]"><Image src="/brand/postcard-icon.svg" alt="" width={34} height={34} /></span></Link></div>
              </div>)}</div>}
            </section>
