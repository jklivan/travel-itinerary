...
export default async function NotificationsPage() {
  ...
  return <div className="max-w-2xl mx-auto px-4 py-6">
    ...
    {notifications.length === 0 ? <p className="rounded-xl border border-[#e3dfd2] bg-[#fffdf6] p-6 text-[#6b7067]">New trips from people you follow and activity on your trips appear here.</p> :
        ...
        {notifications.map(n => <li key={n.id} className={n.readAt ? 'bg-[#faf7ee]' : 'bg-[#fffdf6]'}>
          <form action={openNotification}>
            ...
            <button className="flex w-full items-start gap-3 p-4 text-left hover:bg-[#eee7d9]">
              {n.kind === 'published' ? <Plane className="mt-1 shrink-0 text-[#507c76]" size={20} /> : n.kind === 'comment' ? <MessageCircle className="mt-1 shrink-0 text-[#507c76]" size={20} /> : <Heart className="mt-1 shrink-0 text-[#ad6b57]" size={20} />}
