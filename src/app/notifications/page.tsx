        ...
        {notifications.map(n => <li key={n.id} className={n.readAt ? 'bg-[#faf7ee]' : 'bg-[#fffdf6]'}>
          <form action={openNotification}>
            ...
            <button className="flex w-full items-start gap-3 p-4 text-left hover:bg-[#eee7d9]">
              {n.kind === 'published' ? <Plane className="mt-1 shrink-0 text-[#59694f]" size={20} /> : (n.kind === 'forum_reply' || n.kind === 'forum' || n.kind === 'comment' || n.kind === 'message') ? <MessageCircle className="mt-1 shrink-0 text-[#59694f]" size={20} /> : <Heart className="mt-1 shrink-0 text-[#ad6b57]" size={20} />}
              ...
              {!n.readAt && <span className="mt-2 size-2 shrink-0 rounded-full bg-[#59694f]" aria-label="Unread" />}
            </button>
          </form>
        </li>)}
      </ul>}
