...
export default async function InboxPage() {
  ...
  if (!result.threads) redirect('/login?callbackUrl=%2Ffriends%2Fmessages')
  return <div className="max-w-2xl mx-auto px-4 py-6">
    <Link href="/friends" className="text-sm text-[#8B6F4E]">← Friends</Link>
    <div className="my-5 flex flex-wrap items-center justify-between gap-3 border-b border-[#C4A882] pb-4"><h1 className="font-[family-name:var(--font-playfair)] text-2xl text-[#2C1810]">Private messages</h1><MessageRefresh /></div>
    ...
    <div className="space-y-3">{result.threads.map(thread => <Link key={thread.person.id} href={`/friends/messages/${thread.person.id}`} className="block rounded-xl border border-[#d7cebc] bg-[#FAF7F2] p-4 shadow-sm hover:border-[#507c76] transition-colors">
